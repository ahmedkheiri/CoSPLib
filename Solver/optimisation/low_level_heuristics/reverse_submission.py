...
class ReverseSubmission:
    def apply(self, problem: Problem, solution: Solution) -> None:
        ...
        indexes = np.random.randint(len(solution.getIndSolSubmissions()[track]), size=2)
        ...
        temp: list = solution.getIndSolSubmissions()[track][
            sorted_indexes[0] : sorted_indexes[1]
        ]
        temp.reverse()
        solution.getIndSolSubmissions()[track][
            sorted_indexes[0] : sorted_indexes[1]
        ] = temp
