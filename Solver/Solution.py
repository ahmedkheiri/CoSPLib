...
class Solution:
    def __init__(self, problem):
        ...
        self.__solTracks = [
            [-1 for x in range(self.getProblem().getNumberOfRooms())]
            for y in range(self.getProblem().getNumberOfSessions())
        ]
        self.__solSubmissions = [
            [
                [
                    -1
                    for x in range(
                        self.getProblem().getSession(z).getSessionMaxTimeSlots()
                    )
                ]
                for y in range(self.getProblem().getNumberOfRooms())
            ]
            for z in range(self.getProblem().getNumberOfSessions())
        ]
        self.__indsolSubmissions = [
            [
                self.getProblem().getSubmissionIndex(x.getSubmissionName())
                for x in self.getProblem().getTrack(y).getTrackSubmissionsList()
            ]
            for y in range(self.getProblem().getNumberOfTracks())
        ]
    ...
    def generateEvaluations(self):
        ...
        if self.getProblem().getParameters().getTracksSessionsPenaltyWeight() > 0:
            self.setEvaluation(
                lambda: (
                    self.getProblem().getParameters().getTracksSessionsPenaltyWeight()
        ...
        if self.getProblem().getParameters().getTracksRoomsPenaltyWeight() > 0:
            self.setEvaluation(
                lambda: (
                    self.getProblem().getParameters().getTracksRoomsPenaltyWeight()
        ...
        if self.getProblem().getParameters().getSessionsRoomsPenaltyWeight() > 0:
            self.setEvaluation(
                lambda: (
                    self.getProblem().getParameters().getSessionsRoomsPenaltyWeight()
        ...
        if self.getProblem().getParameters().getSimilarTracksPenaltyWeight() > 0:
            self.setEvaluation(
                lambda: (
                    self.getProblem().getParameters().getSimilarTracksPenaltyWeight()
        ...
        if self.getProblem().getParameters().getNumOfRoomsPerTrackWeight() > 0:
            self.setEvaluation(
                lambda: (
                    self.getProblem().getParameters().getNumOfRoomsPerTrackWeight()
        ...
        if self.getProblem().getParameters().getParallelTracksWeight() > 0:
            self.setEvaluation(
                lambda: (
                    self.getProblem().getParameters().getParallelTracksWeight()
        ...
        if self.getProblem().getParameters().getConsecutiveTracksWeight() > 0:
            self.setEvaluation(
                lambda: (
                    self.getProblem().getParameters().getConsecutiveTracksWeight()
        ...
        if self.getProblem().getParameters().getSubmissionsTimezonesWeight() > 0:
            self.setEvaluation(
                lambda: (
                    self.getProblem().getParameters().getSubmissionsTimezonesWeight()
        ...
        if self.getProblem().getParameters().getSubmissionsOrderWeight() > 0:
            self.setEvaluation(
                lambda: (
                    self.getProblem().getParameters().getSubmissionsOrderWeight()
        ...
        if self.getProblem().getParameters().getSubmissionsSessionsPenaltyWeight() > 0:
            self.setEvaluation(
                lambda: (
                    self.getProblem()
                    .getParameters()
                    .getSubmissionsSessionsPenaltyWeight()
        ...
        if self.getProblem().getParameters().getSubmissionsRoomsPenaltyWeight() > 0:
            self.setEvaluation(
                lambda: (
                    self.getProblem().getParameters().getSubmissionsRoomsPenaltyWeight()
        ...
        if self.getProblem().getParameters().getPresentersConflictsWeight() > 0:
            self.setEvaluation(
                lambda: (
                    self.getProblem().getParameters().getPresentersConflictsWeight()
        ...
        if self.getProblem().getParameters().getAteendeesConflictsWeight() > 0:
            self.setEvaluation(
                lambda: (
                    self.getProblem().getParameters().getAteendeesConflictsWeight()
        ...
        if self.getProblem().getParameters().getChairsConflictsWeight() > 0:
            self.setEvaluation(
                lambda: (
                    self.getProblem().getParameters().getChairsConflictsWeight()
        ...
        if (
            self.getProblem()
            .getParameters()
            .getPresentersConflictsTimeslotLevelWeight()
            > 0
        ):
            self.setEvaluation(
                lambda: (
        ...
        if self.getProblem().getParameters().getAttendeesConflictsTimeSlotWeight() > 0:
            self.setEvaluation(
                lambda: (
                    self.getProblem()
                    .getParameters()
                    .getAttendeesConflictsTimeSlotWeight()
        ...
        for i in range(len(self.getSolTracks())):
            for j in range(len(self.getSolTracks()[i])):
                if self.getSolTracks()[i][j] != -1:
                    pen += self.getProblem().getTracksSessionsPenaltybyIndex(
    ...
    def EvaluateTracksRooms(self) -> int:
        ...
        for i in range(len(self.getSolTracks())):
            for j in range(len(self.getSolTracks()[i])):
                if self.getSolTracks()[i][j] != -1:
                    pen += self.getProblem().getTracksRoomsPenaltybyIndex(
            ...
            for j in range(len(self.getSolTracks()[i])):
                for x in range(j, len(self.getSolTracks()[i])):
                    if (
                        ...
                        pen += self.getProblem().getTracksTracksPenaltybyIndex(
                            self.getSolTracks()[i][j], self.getSolTracks()[i][x]
                        )
        return pen
            ...
            for j in range(len(self.getSolTracks()[i])):
                for x in range(len(self.getSolSubmissions()[i][j])):
                    if self.getSolSubmissions()[i][j][x] != -1:
                        pen += self.getProblem().getSubmissionsTimezonesPenaltybyIndex(
    ...
    def EvaluateSubmissionsOrder(self) -> int:
        ...
        di = {track: [] for track in range(self.getProblem().getNumberOfTracks())}
        session_ts = {
            self.getProblem().getSession(session).getSessionName() + str(ts): []
            for session in range(len(self.getSolTracks()))
            for ts in range(
                self.getProblem().getSession(session).getSessionMaxTimeSlots()
            )
        }
        for session in range(len(self.getSolTracks())):
                ...
                if self.getSolTracks()[session][room] != -1:
                    for ts in range(len(self.getSolSubmissions()[session][room])):
                        ...
                        if (self.getSolSubmissions()[session][room][ts] != -1) and (
                            self.getSolSubmissions()[session][room][ts]
                            not in session_ts[
                                self.getProblem().getSession(session).getSessionName()
                                + str(ts)
                            ]
                        ):
                            session_ts[
        ...
        for ts in session_ts.values():
            for this_sub in range(len(ts) - 1):
                for other_sub in range(this_sub + 1, len(ts)):
                    if (
                        self.getProblem()
                        .getSubmission(ts[this_sub])
                        .getSubmissionTrack()
                        .getTrackName()
                        == self.getProblem()
                        .getSubmission(ts[other_sub])
                        .getSubmissionTrack()
                        .getTrackName()
                    ) and (
                        (
        ...
        for track in range(self.getProblem().getNumberOfTracks()):
            ...
            for sub in di[track]:
                if (
                    self.getProblem().getSubmission(sub).getSubmissionOrder() != order
                ) and (self.getProblem().getSubmission(sub).getSubmissionOrder() != 0):
                    pen += 1
                order += 1
        return pen
    ...
    def EvaluateSubmissionsSessions(self) -> int:
        ...
        for i in range(len(self.getSolTracks())):
            for j in range(len(self.getSolTracks()[i])):
                for x in range(len(self.getSolSubmissions()[i][j])):
                    if self.getSolSubmissions()[i][j][x] != -1:
                        pen += self.getProblem().getSubmissionsSessionsPenaltybyIndex(
                            self.getSolSubmissions()[i][j][x], i
                        )
        return pen
    ...
    def EvaluateSubmissionsRooms(self) -> int:
        ...
        for i in range(len(self.getSolTracks())):
            for j in range(len(self.getSolTracks()[i])):
                for x in range(len(self.getSolSubmissions()[i][j])):
                    if self.getSolSubmissions()[i][j][x] != -1:
                        pen += self.getProblem().getSubmissionsRoomsPenaltybyIndex(
                ...
                for x in range(len(self.getSolSubmissions()[i][j])):
                    if (
                        ...
                        and (
                            len(
                                self.getProblem()
                                .getSubmission(self.getSolSubmissions()[i][j][x])
                                .getSubmissionPresenterConflictsList()
                            )
                            != 0
                        )
            ...
            if len(di[i]) > 1:
                for j in range(len(di[i]) - 1):
                    for z in range(j + 1, len(di[i])):
                        if (
                            self.getProblem().getSubmission(di[i][j][0])
                            in self.getProblem()
                            .getSubmission(di[i][z][0])
                            .getSubmissionPresenterConflictsList()
                    ...
                    if (
                        ...
                        and (
                            len(
                                self.getProblem()
                                .getSubmission(self.getSolSubmissions()[i][j][x])
                                .getSubmissionAttendeeConflictsList()
                            )
                            != 0
                        )
                        and ((self.getSolSubmissions()[i][j][x], j) not in di[i])
        ...
        for i in range(len(self.getSolTracks())):
            for j in range(len(self.getSolTracks()[i])):
                if (self.getSolTracks()[i][j] != -1) and (
                    len(
                        self.getProblem()
                        .getTrack(self.getSolTracks()[i][j])
                        .getTrackChairConflictsList()
                    )
                    != 0
                ):
        ...
        for i in range(len(self.getSolTracks())):
            if len(di[i]) > 1:
                for j in range(len(di[i]) - 1):
                    for z in range(j + 1, len(di[i])):
                        if (
                            self.getProblem().getTrack(di[i][j])
                            in self.getProblem()
                            .getTrack(di[i][z])
                            .getTrackChairConflictsList()
                        ):
                ...
                if len(di[str(session) + str(ts)]) > 1:
                    for j in range(len(di[str(session) + str(ts)]) - 1):
                        for z in range(j + 1, len(di[str(session) + str(ts)])):
                            if (
                                self.getProblem().getSubmission(
                                    di[str(session) + str(ts)][j]
                                )
                                in self.getProblem()
                                .getSubmission(di[str(session) + str(ts)][z])
                                .getSubmissionPresenterConflictsList()
                            ):
                                pen += 1
        return pen
    ...
    def EvaluateExtendedSubmissions(self) -> int:
        ...
        for session in range(len(self.getSolTracks())):
            for room in range(len(self.getSolTracks()[session])):
                for ts in range(len(self.getSolSubmissions()[session][room])):
                    if (self.getSolSubmissions()[session][room][ts] != -1) and (
                        self.getProblem()
                        .getSubmission(self.getSolSubmissions()[session][room][ts])
        ...
        for i in di.values():
            if len(i) != 0:
                temp = set(i)
                for j in temp:
                    if self.getProblem().getSubmission(
                        j
                    ).getSubmissionRequiredTimeSlots() != i.count(j):
                        pen += 10000000
        for i in di2.values():
            if len(i) == 0:
                pen += 1000000000
    ...
    def EvaluateAllSubmissionsScheduled(self) -> bool:
        ...
        for sub in range(self.getProblem().getNumberOfSubmissions()):
            if (sub not in temp) or (
                self.getProblem().getSubmission(sub).getSubmissionRequiredTimeSlots()
                != temp.count(sub)
            ):
                return False
        return True
    ...
    def toExcel(self, file_name="Solution.xlsx"):
        ...
        df = pd.DataFrame(
            self.getSolTracks(),
            index=[
                self.getProblem().getSession(s).getSessionName()
                for s in range(self.getProblem().getNumberOfSessions())
            ],
            columns=[
                self.getProblem().getRoom(r).getRoomName()
            ...
            for t in range(
                ...
                for room in range(self.getProblem().getNumberOfRooms()):
                    if self.getSolSubmissions()[session][room][t] != -1:
                        temp.append(
                            self.getProblem()
                            .getSubmission(self.getSolSubmissions()[session][room][t])
                            .getSubmissionName()
                        )
                    else:
                        temp.append("")
        ...
        df2 = pd.DataFrame(
            ...
            index=[
                self.getProblem().getSession(s).getSessionName()
                for s in range(self.getProblem().getNumberOfSessions())
                for t in range(self.getProblem().getSession(s).getSessionMaxTimeSlots())
            ],
        )
        ...
        for i in range(len(self.getSolTracks())):
            for j in range(len(self.getSolTracks()[i])):
                if self.getSolTracks()[i][j] != -1:
                    if (
                        self.getProblem().getTracksSessionsPenaltybyIndex(
                            self.getSolTracks()[i][j], i
                        )
                        != 0
                    ):
                        p1_list.append(
                            ...
                            .getTrackName()
                            + " - "
                            + self.getProblem().getSession(i).getSessionName()
                        )
                        p1_pen.append(
                            self.getProblem()
                            .getParameters()
                            .getTracksSessionsPenaltyWeight()
                            * self.getProblem().getTracksSessionsPenaltybyIndex(
                                self.getSolTracks()[i][j], i
                            )
                        )
        ...
        for i in range(len(self.getSolTracks())):
            for j in range(len(self.getSolTracks()[i])):
                if self.getSolTracks()[i][j] != -1:
                    if (
                        self.getProblem().getTracksRoomsPenaltybyIndex(
                            self.getSolTracks()[i][j], j
                        )
                        != 0
                    ):
                        p2_list.append(
                            ...
                            .getTrackName()
                            + " - "
                            + self.getProblem().getRoom(j).getRoomName()
                        )
                        p2_pen.append(
                            self.getProblem()
                            .getParameters()
                            .getTracksRoomsPenaltyWeight()
                            * self.getProblem().getTracksRoomsPenaltybyIndex(
                                self.getSolTracks()[i][j], j
                            )
                        )
                ...
                if self.getSolTracks()[i][j] != -1:
                    if self.getProblem().getSessionsRoomsPenaltybyIndex(i, j) != 0:
                        ...
                        p3_pen.append(
                            self.getProblem()
                            .getParameters()
                            .getSessionsRoomsPenaltyWeight()
                            * self.getProblem().getSessionsRoomsPenaltybyIndex(i, j)
                        )
            ...
            for j in range(len(self.getSolTracks()[i])):
                for x in range(j, len(self.getSolTracks()[i])):
                    if (
                        ...
                        if (
                            self.getProblem().getTracksTracksPenaltybyIndex(
                                self.getSolTracks()[i][j], self.getSolTracks()[i][x]
                            )
                            != 0
                        ):
                            p4_list.append(
                                ...
                                + " - "
                                + self.getProblem()
                                .getTrack(self.getSolTracks()[i][x])
                                .getTrackName()
                                + " - "
                                + self.getProblem().getSession(i).getSessionName()
                            )
        ...
        for track in range(self.getProblem().getNumberOfTracks()):
            for session in range(len(temp)):
                ...
                if c > 1:
                    p6_list.append(
                        self.getProblem().getTrack(track).getTrackName()
                        + " - "
                        + self.getProblem().getSession(session).getSessionName()
                    )
                    p6_pen.append(
        ...
        for i in range(len(self.getSolTracks())):
            for j in range(len(self.getSolTracks()[i])):
                for x in range(len(self.getSolSubmissions()[i][j])):
                    if self.getSolSubmissions()[i][j][x] != -1:
                        if (
                            self.getProblem().getSubmissionsTimezonesPenaltybyIndex(
                                self.getSolSubmissions()[i][j][x], i
                            )
                            != 0
                        ):
        ...
        for track in range(self.getProblem().getNumberOfTracks()):
            ...
            for sub in di[track]:
                if (
                    ...
                    p11_list.append(
                        self.getProblem().getSubmission(sub).getSubmissionName()
                    )
                    p11_pen.append(
                        self.getProblem().getParameters().getSubmissionsOrderWeight()
                    )
        ...
        for i in range(len(self.getSolTracks())):
            for j in range(len(self.getSolTracks()[i])):
                for x in range(len(self.getSolSubmissions()[i][j])):
                    if self.getSolSubmissions()[i][j][x] != -1:
                        if (
                            self.getProblem().getSubmissionsSessionsPenaltybyIndex(
                                self.getSolSubmissions()[i][j][x], i
                            )
                            != 0
                        ):
        ...
        for i in range(len(self.getSolTracks())):
            for j in range(len(self.getSolTracks()[i])):
                for x in range(len(self.getSolSubmissions()[i][j])):
                    if self.getSolSubmissions()[i][j][x] != -1:
                        if (
                            self.getProblem().getSubmissionsRoomsPenaltybyIndex(
                                self.getSolSubmissions()[i][j][x], j
                            )
                            != 0
                        ):
                            ...
                            p13_pen.append(
                                self.getProblem()
                                .getParameters()
                                .getSubmissionsRoomsPenaltyWeight()
                                * self.getProblem().getSubmissionsRoomsPenaltybyIndex(
                                    self.getSolSubmissions()[i][j][x], j
                                )
                            )
                ...
                for j in range(len(di[i]) - 1):
                    for z in range(j + 1, len(di[i])):
                        if (
                            ...
                            p16_list.append(
                                self.getProblem().getTrack(di[i][j]).getTrackName()
                                + " - "
                                + self.getProblem().getTrack(di[i][z]).getTrackName()
                            )
                            p16_pen.append(
                                self.getProblem()
                                .getParameters()
                        ...
                        for z in range(j + 1, len(di[str(session) + str(ts)])):
                            if (
                                ...
                                p19_list.append(
                                    str(
                                        self.getProblem()
                                        .getSubmission(di[str(session) + str(ts)][j])
                                        .getSubmissionName()
                                    )
                                    + " - "
                                    + str(
        ...
        with pd.ExcelWriter(file_name) as writer:
            ...
            df2.to_excel(
                writer,
                sheet_name="sol",
                startrow=self.getProblem().getNumberOfSessions() + 2,
                header=False,
            )
            # Objective
        ...
        for session in range(1, self.getProblem().getNumberOfSessions() + 1):
            ...
            for room in range(1, len(file.keys())):
                if file.iloc[session][room] != "":
                    temp.append(
                        self.getProblem().getTrackIndex(file.iloc[session][room])
                    )
                else:
                    temp.append(-1)
            temp2.append(temp)
            ...
            for room in range(1, len(file.keys())):
                ...
                for ts in range(
                    ...
                    if file.iloc[index][room] != "":
                        temp.append(
                            self.getProblem().getSubmissionIndex(file.iloc[index][room])
                        )
                    else:
                        temp.append(-1)
                index = self.getProblem().getNumberOfSessions() + 1 + sum_ts
...
class RandomInd(InitialSolution):
    def __init__(self, problem):
        ...
        temp2 = [
            sub
            for sub in range(self.getProblem().getNumberOfSubmissions())
            if self.getProblem().getSubmission(sub).getSubmissionRequiredTimeSlots()
            == 1
        ]
        sessions = [
            session for session in range(self.getProblem().getNumberOfSessions())
        ]
